...
def train(
    ...
    for epoch in range(num_epochs):
        ...
        for x, _ in train_loader:
            ...
            # discriminator step
            D.zero_grad() # In the discriminator step, you should zero gradients in the discriminator
            fake_images = G(sample_noise(batch_size, noise_size).to(device)).view(batch_size, input_channels, img_size, img_size) # and sample noise to generate a fake data batch using the generator. Calculate
            real_data_disc_output, fake_data_disc_output = D(real_images), D(fake_images.detach()) # the discriminator output for real and fake data, 
            d_error = discriminator_loss(real_data_disc_output, fake_data_disc_output) # and use the output to compute discriminator loss.
            d_error.backward() #  Call backward() on the loss output and take an optimizer
            D_solver.step() # step for the discriminator.

            # generator step
            G.zero_grad() # (2) For the generator step, you should once again zero gradients in the generator
            fake_data_batch = D(fake_images) # and sample noise to generate a fake data batch. Get the discriminator output
            g_error = generator_loss(fake_data_batch) # for the fake data batch and use this to compute the generator loss. Once again
            g_error.backward() # call backward() on the loss 
            G_solver.step() # and take an optimizer step.
